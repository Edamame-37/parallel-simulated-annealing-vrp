# Makefile untuk Parallel Simulated Annealing CVRP Menggunakan MPI
# Compiler dan flags
CC = mpicc
CFLAGS = -Wall -O2
LDFLAGS = -lm

# Nama target binary
TARGET = vrp_sa
SRC = vrp_sa_mpi.c

# Dataset default
DATA_SMALL = data/dataset_small.txt
DATA_MEDIUM = data/dataset_medium.txt

.PHONY: all clean run-small run-medium help

# Target default: kompilasi program
all: $(TARGET)

$(TARGET): $(SRC)
	$(CC) $(CFLAGS) -o $(TARGET) $(SRC) $(LDFLAGS)

# Menjalankan dengan dataset kecil menggunakan 4 prosesor
run-small: $(TARGET)
	mpirun -np 4 ./$(TARGET) $(DATA_SMALL)

# Menjalankan dengan dataset medium menggunakan 4 prosesor
run-medium: $(TARGET)
	mpirun -np 4 ./$(TARGET) $(DATA_MEDIUM)

# Membersihkan file binary hasil kompilasi
clean:
	rm -f $(TARGET) $(TARGET).exe

# Menampilkan bantuan penggunaan Makefile
help:
	@echo "Perintah yang tersedia:"
	@echo "  make            - Mengompilasi program (mpicc)"
	@echo "  make run-small  - Menjalankan dengan 4 prosesor pada dataset kecil (15 pelanggan)"
	@echo "  make run-medium - Menjalankan dengan 4 prosesor pada dataset menengah (50 pelanggan)"
	@echo "  make clean      - Menghapus binary keluaran"
